...
is set to frontend/ — Vercel only auto-detects Python Functions inside the
Root Directory, and Next.js framework auto-detection also requires
package.json to be at the Root Directory. The backend package itself was
not moved; only this entrypoint's location changed, so the relative
lookup below goes up two levels (out of frontend/api/, out of frontend/)
to reach the sibling backend/ directory.
"""
