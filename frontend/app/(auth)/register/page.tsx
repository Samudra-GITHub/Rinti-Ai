...
function clientValidate(name: string, email: string, password: string, confirm: string): string | null {
...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const validationError = clientValidate(name, email, password, confirmPassword);
    if (validationError) {
  ...
  return (
    <div className="glass-panel relative w-full max-w-sm rounded-2xl p-8">
      <div className="flex flex-col items-center text-center">
        <RintiOrb size="md" />
        <h1 className="mt-4 text-lg font-semibold text-white/95">Create your account</h1>
        <p className="mt-1 text-sm text-white/45">A few details and you&apos;re in.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-4" noValidate>
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="confirm-password" className="text-xs font-medium text-white/55">
            Confirm password
          </label>
          <input
            id="confirm-password"
      ...
      <p className="mt-6 text-center text-sm text-white/40">
        ...
        <Link href="/login" className="text-purple-300 hover:text-purple-200 focus-ring">
          Sign in
        </Link>
      </p>
    </div>
